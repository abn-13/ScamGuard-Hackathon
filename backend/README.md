...
### Task 2 — Real Telegram family alerts — built, tested, intentionally not exposed in the app
**Files:** `app/alerts.py`, `app/telegram_link.py`, Android's `TelegramLinkActivity`
Linking is fully automated: `POST /family-members` (without a `telegram_chat_id`) generates a one-time code and returns it plus a `https://t.me/<bot>?start=<code>` deep link (needs `TELEGRAM_BOT_USERNAME` set to build the link; the raw code still works without it). A family member sends that to the bot, and `POST /family-members/{id}/link-telegram` short-polls Telegram's `getUpdates` once and saves their `chat_id` if the code matches. `app/alerts.py` sends to any member who has one.

**Deliberately not wired into the app's registration flow** -- SMS + on-device notification (`GuardianAlerter`/`RiskNotifier`, both already working, see `SETUP.md` § Risk alerts) already cover alerting, and the team chose not to expose a third channel in the UI. The code is left in place, not deleted, in case that changes later -- re-enabling it is just restoring the `TelegramLinkActivity` handoff in `RegistrationActivity.submit()` (see the comment there).

To actually enable sending if it does get re-wired: create a bot via [@BotFather](https://t.me/BotFather) (a couple of minutes, gives you a token + username), set `TELEGRAM_BOT_TOKEN` (and optionally `TELEGRAM_BOT_USERNAME` for the deep link) in `.env`.
